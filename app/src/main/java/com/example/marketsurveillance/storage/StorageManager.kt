package com.example.marketsurveillance.storage
...
//            btm.compress(Bitmap.CompressFormat.WEBP_LOSSY,80,it)
            val btm = BitmapFactory.decodeByteArray(imageByteArray,0,imageByteArray.size)
            context.openFileOutput(UUID.randomUUID().toString().plus(".jpeg"), Context.MODE_PRIVATE).use {
                btm.compress(Bitmap.CompressFormat.JPEG, 80, it)
        }
    }
}