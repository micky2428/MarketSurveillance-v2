...
import com.example.marketsurveillance.textdetector.preference.PreferenceUtils.getCameraPreviewSizePair
...
class CameraSource(protected var activity: Activity, private val graphicOverlay: GraphicOverlay) {
    ...
    fun setZoom(zoomRatio: Float): Boolean {
        ...
        parameters.zoom = getZoomValue(parameters, zoomRatio)
    ...
    companion object {
         ...
         *
         *
         * According to the camera API, [Parameters.getZoomRatios] will return a list of zoom
         * ratios with length [Parameters.getMaxZoom]+1. Each of this value indicates a actual
         * zoom ratio of the camera.
         *
         *
         * E.g. Assume [Parameters.getZoomRatios] return `[100, 114, 131, 151, 174, 200,
         * 234, 268, 300]`, where [Parameters.getMaxZoom]=8. It means, `setZoom(0)` will
         * actual perform 1.00x to the camera, `setZoom(1)` will actual perform 1.14x to the camera,
         * `setZoom(2)` will actual perform 1.31x to the camera, ..., `setZoom(8)` will actual
         * perform 3.00x to the camera.
         *
         * @param params The parameters of the camera.
         * @param zoomRatio The target zoom ratio.
         * @return The maximum zoom value that will not exceed the target `zoomRatio`.
         */
        private fun getZoomValue(params: Camera.Parameters, zoomRatio: Float): Int {
            val zoom = (Math.max(zoomRatio, 1f) * 100).toInt()
            val zoomRatios = params.zoomRatios
            val maxZoom = params.maxZoom
            for (i in 0 until maxZoom) {
                if (zoomRatios[i + 1] > zoom) {
                    return i
                }
            }
            return maxZoom
